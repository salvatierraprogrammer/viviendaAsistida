...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    try {
      ...
      const fetchedUserData = userDoc.data(); // Use a different variable name
      ...
      if (fetchedUserData && fetchedUserData.userRole) {
        ...
        } else if (userRole === '2') { // Corrected comparison to string
         
          navigation.navigate('SelectHouseScreen', { fetchedUserData: fetchedUserData});
